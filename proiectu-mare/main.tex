...
\subsection{Stan Victor's}

\textbf{Vulerability 07: Web Server Transmits Cleartext Credentials CWE-319}
...
\textbf{Vulerability 08: Web Application Potentially Vulnerable to Clickjacking CVE-2025-64387.}
...
7 & Cleartext credentials & \textbf{file:} \texttt{app.py} \newline \textbf{function:} \texttt{login} \newline \textbf{credentials:} Password \\ \hline
8 & Potential clickjacking & \textbf{file:} \texttt{app.py} \newline \textbf{function:} \texttt{any tbh} \newline \textbf{integrity:} User browser session, frontend UI \\ \hline
\end{tabular}
\end{table}
...
7 & Cleartext credentials & User Passwords & Confidentiality \\ \hline
8 & Potential Clickjacking & User Session & Integrity \\ \hline
\end{tabular}
\end{table}
...
\textbf{Technique Employed:} Specification-Based Testing (Black Box Inspection) and some manual inspection, as well as automated scripts. \\

\begin{longtable}{|p{2cm}|p{3.5cm}|p{3.5cm}|p{3.5cm}|p{2cm}|}
\hline
\textbf{TC ID} & \textbf{Input / Action} & \textbf{Expected Output} & \textbf{Actual Result} & \textbf{Feature} \\
\hline
TC-AUTH-01 & Navigate to \texttt{http://www.vulnbank.org/login} and inspect the page protocol. & The browser should automatically redirect to \texttt{https://}. & The page loads over \texttt{http://} without redirection. & Login \\
\hline
TC-AUTH-02 & Inspect the HTML source code of the Login form \texttt{<form>} tag. & The \texttt{action} attribute should point to an absolute \texttt{https://} URL. & The \texttt{action} attribute is relative (or \texttt{http}), inheriting the insecure protocol. & Login \\
\hline
TC-AUTH-03 & Submit dummy credentials (user: \textit{test}, pass: \textit{Secret123}) while monitoring network traffic. & The payload should be encrypted (not readable in plain text). & The payload is visible in cleartext in the POST body. & Login / Register \\
\hline
\end{longtable}
...
\textbf{TC ID} & \textbf{Input / Action} & \textbf{Expected Output} & \textbf{Actual Result} & \textbf{Feature} \\
\hline
TC-CJ-01 & Send a GET request to the target URL and inspect HTTP Response Headers. & The header \texttt{X-Frame-Options} should be present with values \texttt{DENY} or \texttt{SAMEORIGIN}. & The \texttt{X-Frame-Options} header is completely missing from the response. & Global HTTP Headers \\
\hline
TC-CJ-02 & Send a GET request to the target URL and inspect \texttt{Content-Security-Policy} (CSP). & The CSP header should contain the \texttt{frame-ancestors} directive restricting framing sources. & The \texttt{Content-Security-Policy} header is missing or lacks the \texttt{frame-ancestors} directive. & Global HTTP Headers \\
\hline
TC-CJ-03 & Create a local HTML file containing an \texttt{<iframe src="http://www. vulnbank.org">} and open it in a browser. & The browser should refuse to render the content within the frame (e.g., "refused to connect"). & The target website renders successfully inside the iframe without errors. & UI / Rendering \\
\hline
\end{longtable}
...
\begin{enumerate}
    \item \textbf{Setup:} open a browser at
    \item \textbf{Inspect:} Open the develop tools network tab as sending the request.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Figures/nessus/credential-unencrypted.png}
    \caption{unencrypted password}
\end{figure}
...
\subsubsection{Vulnerability 07:}
...
\begin{lstlisting}[language=Python, caption=Cleartext Submission Verification Script]
import requests
import sys
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{Figures/nessus/setting-proxy.jpeg}
    \caption{Setting my proxy}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{Figures/nessus/fillling-in-the-form.jpeg}
    \caption{Filling in my form}
\end{figure}
...
\subsubsection{Vulerability 08, Clickjacking:}
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{Figures/nessus/theghostly-figure.png}
    \caption{Opacity set to 0.5, for development purposes}
\end{figure}

Now this would work if the credentials were already preffilled inside the form, perhaps from a query of the type \verb|.../?user=<username>&pass=<password>| or something.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.45\textheight,keepaspectratio]{Figures/nessus/reaping-bepefits.png}
    \caption{Opacity set to zero, clicking the restore session actually sends the form.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{Figures/snyk-report-before-fix.png}
\caption{\label{fig:initial-report} Initial Snyk Scan Report Summary }
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{Figures/snyk-report-after-fix.png}
