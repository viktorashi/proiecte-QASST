...
\subsection{Application 2: GitLab}
\label{sec:gitlab}
...
\begin{table}[h]
    ...
    Reports Resolved & $>$\textbf{1,970} & High efficiency and capacity \\
    Response Efficiency & \textbf{High (\(\sim 7\)h) } & Time to first response. Industry leading \\
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/gitLabRewardSummary.png}
    \caption{Reward summary for GitLab on HakerOne \cite{GitLabHackerOnePolicy}}.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/gitLabPyalodExample.png}
    \caption{Example of modified JSON Payload to exploit vulnerability. \cite{HackerOne2293343}}.
\label{fig:gitlab-example}
\end{figure}
\end{exploitcontext}
